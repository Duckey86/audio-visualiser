...
- 涉及文件：providers/spotify-provider.js、server.js、desktop/main.js、public/index.html、public/spotify-integration.js、public/spotify-integration.css、docs/SPOTIFY_SETUP.md、README.md、PRIVACY.md、package.json。
- 关键参数/实现：Authorization Code with PKCE；loopback 回调使用 127.0.0.1 动态端口；本地 .spotify-auth.json 保存 Client ID/令牌；支持搜索、喜欢的音乐、播放状态和 Spotify Connect 播放/暂停/进度控制；Spotify 播放态使用不读取音频的环境视觉。
- 禁止回退或改坏的点：不要加入 Client Secret；不要下载、代理或分析 Spotify 音频；不要承诺 Spotify 和网易云/QQ 一样提供直链；不要移除 Spotify 模式的 ambient 标识和无音频分析边界。
