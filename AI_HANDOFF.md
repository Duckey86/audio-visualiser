...
- 新增 Spotify PKCE 登录、本地令牌存储、状态、搜索、喜欢的音乐、歌单数据接口，以及播放/暂停/恢复/上一首/下一首/进度控制接口。
- 前端新增 SP 搜索来源、Spotify 设置/连接窗口、右上角连接状态、喜欢的音乐入口和 Spotify 环境视觉模式；Spotify 音频不进入 Mineradio 节拍分析器。
- 新增 docs/SPOTIFY_SETUP.md，并更新 README、PRIVACY、package.json、.gitignore。
- 已通过 server/main/preload/provider/Spotify 前端脚本语法解析、public/index.html 内联脚本解析，以及 Provider 配置与 PKCE S256 登录 URL 的模拟验证。
- 当前工作区未安装 node_modules，且浏览器测试权限未完成，因此尚未做真实 Spotify 账号授权与 Electron 实机 UI 验证；首次实机测试需按 docs/SPOTIFY_SETUP.md 配置 Client ID。
